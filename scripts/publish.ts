...
const publishPackage = async (packageName: string) => {
  ...
  console.log(`\n📦 Publishing @rfetch/${packageName}...`);

  const { stdout, stderr, exitCode } = await $`npm publish`
    .cwd(packageDir)
    .nothrow();

  if (exitCode !== 0) {
    console.error(`❌ Failed to publish @rfetch/${packageName}`);
    console.error(stderr.toString());
    return false;
  }

  console.log(stdout.toString());
  console.log(`✅ Published @rfetch/${packageName}`);
  return true;
};

const main = async () => {
  console.log("🚀 Publishing RFetch packages to npm...");
  console.log("========================================\n");
  ...
  for (const pkg of PACKAGES) {
    ...
    if (!distExists) {
      console.error(
        `❌ Package @rfetch/${pkg} is not built. Run 'bun run build' first.`
      );
      process.exit(1);
    }
  }
